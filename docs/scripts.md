...
| `fm-scout-dag.sh`        | Bounded scout DAG runtime: BFS-expand `(depends: ...)` from a root, dispatch leaves in parallel via `fm-spawn.sh`, wait the frontier, and advance layer by layer with one captain notification per layer (manual dispatch remains the fall-back) |
